 ...
 */
fun abs(v: List<Any>): Double = TODO()
...
fun times(a: List<Double>, b: List<Double>): Double {
    ...
    for (i in 0 until a.size) scalarProduct += a[i] * b[i]
...
fun polynom(p: List<Double>, x: Double): Double {
    var pFromX = 0.0
    for (i in 0 until p.size) pFromX += p[i] * Math.pow(x, i.toDouble())
    return pFromX
}
...
fun convert(n: Int, base: Int): List<Int> {
    val listBase = mutableListOf<Int>()
    var num = n
    while (num >= 0) {
        listBase.add(num % base)
        num /= base
        if (num == 0) break
    }
    return listBase.reversed()
}
...
fun convertToString(n: Int, base: Int): String {
    ...
    for (i in 0 until newList.size) {
        if (newList[i] > 9) line.append((newList[i] + numFirstLetterInASCII).toChar())
        else line.append(newList[i])
    }
...
fun decimalFromString(str: String, base: Int): Int {
    ...
    for (i in 0..str.length - 1) {
        if (str[i] in 'a'..'z') newList.add(str[i] - 'a' + 10)
            else newList.add(str[i] - '0')
    }
...
fun roman(n: Int): String {
    ...
    while(copyN > 0){
        if(copyN >= numbersInDecimal[countListChar]) {
            ...
            copyN -= count * numbersInDecimal[countListChar]
            for(i in 1..count) line.append(numbersInRoman[countListChar])
            countListChar -= 1
        }
        else countListChar -= 1
    }
    return line.toString()
}
